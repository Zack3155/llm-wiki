...
1. 内容讨论站点身份、脚本行为、cron、搜索路由、MkDocs、健康检查 → 进入 `general` 或 `wiki-ops`，不得进入 `小说考据`。
2. 内容讨论小说角色、概念、终局、身份冲突、原文证据 → 进入 `小说考据`。
...
5. 新领域出现时，先建 topic 目录、`_index.md` 和根索引链接，再写具体页面；不得为了复用现有目录而把新领域硬塞进 `小说考据`。
